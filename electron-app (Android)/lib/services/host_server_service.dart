...
class HostServerService {
  ...
  /// Mirrors server.py's control_allowed flag — gates whether connecting
  /// clients get "ALLOWED\n" (and can then send commands) or "DENIED\n"
  /// (connection closed immediately). Toggle from Server Mode's UI.
  ...
  void _handleControlClient(Socket client) {
    ...
    // Same handshake as server.py: tell the client immediately whether
    // control is currently allowed, and refuse the connection if not.
    if (!controlAllowed) {
      client.write('DENIED\n');
      client.destroy();
      _log('Control client ${client.remoteAddress.address} rejected — control disabled');
      return;
    }
    client.write('ALLOWED\n');
  ...
  // Plain-text protocol matching server.py exactly (mouse_move:dx,dy,
  // mouse_click, shortcut:name, key:text, etc.) — NOT JSON. A remote peer
  // (Electron or another Flutter client) speaks this same protocol
  // regardless of which side is hosting.
  void _handleControlMessage(String line) {
    try {
      // TODO: forward to a real input-injection layer here. Android has no
      // equivalent of xdotool/pyautogui for injecting system-wide input;
      // that requires the AccessibilityService API. This stub just logs
      // what it received so the networking + permission gate can be
